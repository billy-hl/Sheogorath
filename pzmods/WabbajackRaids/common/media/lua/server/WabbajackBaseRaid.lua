...
-- One setting shared across the toolkit; see world.realMinutesPerDay.
...
Global rather than local for the same reason the old WabbajackSweep_start was: the
...
--[[
The menu's channel for this mod.

Moved here from WabbajackSiege.lua when the toolkit was split. Each mod now owns
the commands that drive it, so a mod that is not installed simply has no
listener and the menu never offers the entry - which is a better answer than the
old "the base raid module is not installed on this server" branch sitting in
another mod's file.

Staff is re-checked here. The menu hiding itself is not a permission check.
]]
Events.OnClientCommand.Add(function(module, command, player, args)
    if command ~= "baseRaid" then return end
    if not WabbajackCore.allow("WabbajackBaseRaid", module, player) then return end
