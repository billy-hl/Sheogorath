--[[
DISABLED 2026-08-19 - see the banner in client/WabbajackRaidGate.lua. The raid
protection was buggy in play and all three of its parts are commented out
together: this file, the client gate, and the "Raid protection" settings in
shared/WabbajackSettingsSchema.lua.

With this commented out the server publishes no open set at all, which is the
protected direction: the client gate is off too, so every safehouse falls back to
vanilla's own rule and stays closed to non-members.

The original header is kept below, commented, because it is the record of why the
rule was written the way it was. To restore, strip the leading "-- " from every
line below this block.
]]
...
--         broadcast(published or recompute() or "", player)
--         return
--     end
--
--     if command == "gate" then
--         local how = tostring((args and args.how) or "unknown")
--         local line = tostring(player:getUsername()) .. " gate=" .. how
--         if how == "failed" then
